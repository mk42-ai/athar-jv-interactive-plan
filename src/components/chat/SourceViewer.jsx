...
function SourcePdf({ view }) {
  ...
  return <div className="source-pdf" ref={holder} data-testid="source-pdf" data-state={view.preview?.available ? state : 'unavailable'}>
    {!view.preview?.available ? <p role="status">This source preview is unavailable. The original and extracted source records remain accessible; no substitute slide is displayed.</p> : <>
      ...
      {state==='error'&&<p role="alert">The document preview could not be opened. Retry the source below.</p>}
      <div className="source-pdf-tools"><button className="btn small" onClick={()=>setZoom(1)} aria-label="Fit document width">Fit width</button><button className="btn small" onClick={()=>setZoom(z=>Math.max(0.5,z-0.25))} aria-label="Zoom out">−</button><span>{Math.round(zoom*100)}%</span><button className="btn small" onClick={()=>setZoom(z=>Math.min(3,z+0.25))} aria-label="Zoom in">+</button></div><div className="source-pdf-scroll" tabIndex={0} role="region" aria-label="Original page preview, scroll when zoomed"><div className="source-pdf-page"><canvas ref={canvas} aria-label={`Source ${view.kind==='pptx'?'slide':'page'} ${view.previewPage}`} data-testid="source-pdf-canvas" />
      ...
      </div><details className="source-page-text"><summary>Read page text</summary><div data-testid="source-page-text">{pageText}</div></details>
      <p className="source-note">{view.preview.derivative ? 'Read-only rendering of the original PowerPoint. Slide count and SHA-256 verified.' : 'Original PDF — bytes verified by SHA-256.'} Use Read page text for small print; zoom for original layout.</p>
    </>}
  </div>;
}
...
export default function SourceViewer({source,citationId,onClose,libraryMode=false}) {
  ...
  const locationLabel=view?.location.sheet?`${view.location.sheet}!${view.location.range}`:view?.location.page?`Page ${view.location.page}`:view?.location.slide?`Slide ${view.location.slide}`:'';
  ...
  return <div className="source-viewer" data-testid="source-viewer" data-kind={view?.kind||''} data-location={locationLabel} aria-busy={busy}>
    ...
    {view&&<>
      {view.renderer==='text'?<p className="source-note">Original package metadata / supplemental OCR · {view.location.part}</p>:view.kind==='xlsx'?<form className="source-nav" onSubmit={e=>{e.preventDefault();navigate({sheet,range});}}><label>Worksheet<select value={sheet} aria-disabled={busy} onChange={e=>{if(!busy){setSheet(e.target.value);navigate({sheet:e.target.value});}}} data-testid="source-sheet-select">{view.availableLocations.sheets.map(s=><option key={s.name} value={s.name}>{s.name}{s.state==='hidden'?' (hidden in original)':''}</option>)}</select></label><label>Cell range<input value={range} aria-disabled={busy} onChange={e=>{if(!busy)setRange(e.target.value);}} aria-label="Source cell range" data-testid="source-range-input" /></label><button className="btn small" type="submit" aria-disabled={busy}>Go to cells</button><button className="btn small" type="button" onClick={()=>moveRows(-1)} aria-disabled={!canMoveRows(-1)} aria-label="Previous source rows">↑</button><button className="btn small" type="button" onClick={()=>moveRows(1)} aria-disabled={!canMoveRows(1)} aria-label="Next source rows">↓</button><button className="btn small" type="button" onClick={()=>moveColumns(-1)} aria-disabled={busy} aria-label="Previous source columns">←</button><button className="btn small" type="button" onClick={()=>moveColumns(1)} aria-disabled={busy} aria-label="Next source columns">→</button><button className="btn small" type="button" onClick={()=>moveEdge(false)} aria-disabled={busy}>First rows</button><button className="btn small" type="button" onClick={()=>moveEdge(true)} aria-disabled={busy} data-testid="source-last-rows">Last rows</button><p className="source-range-help">Every cell is accessible. Enter any range (up to 200 cells), or move through row/column windows.</p></form>:<div className="source-nav" role="group" aria-label="Source page navigation"><button className="btn small" aria-disabled={busy||current<=1} onClick={()=>{if(current>1)navigate({[key]:current-1});}} aria-label={`Previous source ${key}`}>←</button><label>{key==='slide'?'Slide':'Page'}<select aria-label={`Source ${key}`} value={busy ? target[key]||current : current} aria-disabled={busy} onChange={e=>navigate({[key]:Number(e.target.value)})} data-testid="source-page-select">{Array.from({length:max},(_,i)=><option key={i} value={i+1}>{i+1} of {max}</option>)}</select></label><button className="btn small" aria-disabled={busy||current>=max} onClick={()=>{if(current<max)navigate({[key]:current+1});}} aria-label={`Next source ${key}`}>→</button></div>}
      {!libraryMode && <button className="btn small source-return" onClick={()=>navigate({})} aria-disabled={busy} data-testid="source-return-citation">Return to cited location</button>}
      {view.kind==='xlsx' && view.renderer==='workbook' && <p className="source-dimensions" data-testid="source-sheet-dimensions">Full worksheet: {view.availableLocations.sheets.find(s=>s.name===sheet)?.bounds.range} · Window: {view.location.range}</p>}
      {view.renderer==='text'?<pre className="source-metadata-text">{view.citationText}</pre>:view.renderer==='workbook'?<Worksheet view={view}/>:<SourcePdf view={view}/>}
      <details className="source-excerpt"><summary>Exact source excerpt · {source.label}</summary><blockquote>{source.excerpt}</blockquote></details>
