...
export default function SourceViewer({source,citationId,onClose,onAuthRequired}) {
  ...
  return <div className="source-viewer" data-testid="source-viewer" data-kind={view?.kind||''} data-location={locationLabel} aria-busy={busy}>
    ...
    {view&&!busy&&!error&&<>
      {view.kind==='xlsx'?<form className="source-nav" onSubmit={e=>{e.preventDefault();setTarget({sheet,range});}}><label>Worksheet<select value={sheet} onChange={e=>{setSheet(e.target.value);setTarget({sheet:e.target.value});}} data-testid="source-sheet-select">{view.availableLocations.sheets.map(s=><option key={s.name} value={s.name}>{s.name}{s.state==='hidden'?' (hidden in original)':''}</option>)}</select></label><label>Cell range<input value={range} onChange={e=>setRange(e.target.value)} aria-label="Source cell range" data-testid="source-range-input" /></label><button className="btn small" type="submit">Go to cells</button><button className="btn small" type="button" onClick={()=>moveRows(-1)} aria-label="Previous source rows">↑</button><button className="btn small" type="button" onClick={()=>moveRows(1)} aria-label="Next source rows">↓</button></form>:<div className="source-nav" role="group" aria-label="Source page navigation"><button className="btn small" disabled={current<=1} onClick={()=>setTarget({[key]:current-1})} aria-label={`Previous source ${key}`}>←</button><label>{key==='slide'?'Slide':'Page'}<select aria-label={`Source ${key}`} value={current} onChange={e=>setTarget({[key]:Number(e.target.value)})} data-testid="source-page-select">{Array.from({length:max},(_,i)=><option key={i} value={i+1}>{i+1} of {max}</option>)}</select></label><button className="btn small" disabled={current>=max} onClick={()=>setTarget({[key]:current+1})} aria-label={`Next source ${key}`}>→</button></div>}
      <button className="btn small source-return" onClick={()=>setTarget({})} data-testid="source-return-citation">Return to cited location</button>
    ...
    <div className="source-footer"><button className="btn small" onClick={()=>setRetry(n=>n+1)}>Retry source</button>{originalUrl&&<a href={originalUrl} className="btn small" download data-testid="citation-original">Download original</a>}</div>
  </div>;
}
