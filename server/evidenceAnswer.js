...
export function buildEvidencePrompt({ question, retrieved, history = [], documentId = retrieved?.documentId ?? 'all' } = {}) {
  ...
  for (const [id, text] of visible) {
    ...
    evidence.push({ id, documentId: record.documentId, kind: record.kind, label: record.label.slice(0, 2000), location: record.location, text, provenance: { originalSha256: record.documentId, ocr: record.metadata?.ocr === true, manuallyVerified: record.metadata?.manuallyVerified ?? null, evidenceOrigin: record.metadata?.evidenceOrigin || record.metadata?.corpusLayer || record.extractionKind || record.kind } });
  }
  const documents = (retrieved?.documents || []).filter(doc => documentId === 'all' || doc.id === documentId).map(doc => ({
    id: doc.id, sha256: doc.sha256, versionLabel: doc.versionLabel || doc.sha256?.slice(0, 10), aliases: doc.aliases || [], title: String(doc.title).slice(0, 1000), kind: doc.kind, status: doc.status,
...
export function validateEvidenceAnswer(raw, { retrieved, question = '' } = {}) {
  ...
  for (const reference of references) if (!seen.has(reference.id)) {
    ...
    const source = sources.records.get(reference.id);
    const doc = retrieved.documents?.find(doc => doc.id === source.documentId);
    citations.push({ id: source.id, documentId: source.documentId, sha256: doc?.sha256 || source.documentId, versionLabel: doc?.versionLabel || source.documentId.slice(0, 10), label: (doc ? doc.title + ' · SHA ' + doc.id.slice(0, 10) + ' · ' : '') + source.label, location: { ...source.location }, url: `/api/citations/${encodeURIComponent(source.id)}` });
  }
